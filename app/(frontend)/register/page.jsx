...
function RegisterFormContent() {
  ...
  useEffect(() => {
    ...
    if (hasCode || (hashParams && hashParams.get('access_token'))) {
      ...
      setMessage({
        type: 'error',
        text: searchParams.get('error_description') || 'Google authentication failed. Please try again.'
      })
      router.replace('/register')
    }
  }, [searchParams, router])
  ...
  // Manual Email/Password Submission
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: data.message || 'Account created successfully! Please visit the login page to login.' })
      setName('')
      setEmail('')
      setPassword('')
    } catch (error) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 max-w-[1200px] w-full mx-auto items-center p-5">
      
      {/* Left Column Graphic Layout */}
      ...
      <div className="lg:col-span-6 flex flex-col justify-center max-w-[400px] mx-auto w-full">
          ...
          <div className={`mb-6 p-4 rounded text-xs font-medium border transition-all ${
            ...
            {message.type === 'success' && (
              <div className="mt-2.5 pt-2 border-t border-emerald-200/60">
                <Link href="/login" className="text-black underline font-bold hover:opacity-80 transition-opacity">
                  Go to Login Page →
                </Link>
              </div>
            )}
        ...
        {/* Manual Input Execution Fields */}
        <form className="space-y-5" onSubmit={handleSubmit}>
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-[#4c4546] uppercase tracking-wider">Full Name</label>
            <input 
              type="text"
              required
              value={name}
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-[#4c4546]">
            ...
            <Link className="text-black font-semibold hover:underline" href="/login">Sign In</Link>
          </p>
        </div>

        <div className="mt-8 text-center">
          <p className="text-xs text-[#4c4546] leading-relaxed">
            By creating an account, you agree to our <a className="underline" href="#">Terms of Service</a> and <a className="underline" href="#">Privacy Policy</a>. We value your silence and privacy.
          </p>
        </div>
      </div>
